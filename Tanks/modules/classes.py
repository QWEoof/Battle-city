...
class Bullet(pygame.Rect):
    def __init__(self, x, y):
        ...
        self.image = pygame.image.load(os.path.join(PATH, 'bull.png'))
...
class Player(Panzar):
    def __init__(self, x, y):
        super().__init__(x, y)
        self.image =  pygame.image.load(os.path.join(PATH, 'tank_our.png'))
        self.image =  pygame.transform.scale(self.image, (STEP, STEP))
    def move(self):
...
class Player2(Panzar):
    def __init__(self, x, y):
        super().__init__(x, y)
        self.image =  pygame.image.load(os.path.join(PATH, 'tank_enemy.png'))
        self.image =  pygame.transform.scale(self.image, (STEP, STEP))
    def move(self):
        keys = pygame.key.get_pressed()
        if keys[pygame.K_UP]:
